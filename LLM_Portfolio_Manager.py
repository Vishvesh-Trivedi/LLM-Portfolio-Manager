...
ALPACA_API_BASE_URL = os.getenv('APCA_API_BASE_URL', 'https://paper-api.alpaca.markets').strip()
SCREENER_LIVE_BROKER = os.getenv('SCREENER_LIVE_BROKER', '').strip().lower() in ('1', 'true', 'yes', 'on')
ALPACA_PAPER_CAPITAL = 100_000.00

_ALPACA_DATA_CLIENT = [None]
_ALPACA_TRADING_CLIENT = [None]
_ALPACA_TRADE_STREAM = [None]
_ALPACA_TRADE_STREAM_THREAD = [None]
_ALPACA_TRADE_STREAM_LOCK = threading.Lock()
ALPACA_STREAM_UPDATES = os.getenv('ALPACA_STREAM_UPDATES', '0').strip().lower() in ('1', 'true', 'yes', 'on')


def _alpaca_ready():
    return bool(_ALPACA_SDK_AVAILABLE and ALPACA_API_KEY and ALPACA_SECRET_KEY)


def _alpaca_data_enabled():
    return _alpaca_ready()


def _alpaca_trading_enabled():
    return _alpaca_ready() and SCREENER_LIVE_BROKER


def _alpaca_data_client():
    if not _alpaca_data_enabled():
        return None
    if _ALPACA_DATA_CLIENT[0] is None:
        try:
            _ALPACA_DATA_CLIENT[0] = StockHistoricalDataClient(api_key=ALPACA_API_KEY, secret_key=ALPACA_SECRET_KEY)
        except TypeError:
            _ALPACA_DATA_CLIENT[0] = StockHistoricalDataClient(ALPACA_API_KEY, ALPACA_SECRET_KEY)
    return _ALPACA_DATA_CLIENT[0]


def _alpaca_trading_client():
    if not _alpaca_trading_enabled():
        return None
    if _ALPACA_TRADING_CLIENT[0] is None:
        try:
            _ALPACA_TRADING_CLIENT[0] = TradingClient(api_key=ALPACA_API_KEY, secret_key=ALPACA_SECRET_KEY, paper=True, url_override=ALPACA_API_BASE_URL)
        except TypeError:
            try:
                _ALPACA_TRADING_CLIENT[0] = TradingClient(api_key=ALPACA_API_KEY, secret_key=ALPACA_SECRET_KEY, paper=True)
            except TypeError:
                _ALPACA_TRADING_CLIENT[0] = TradingClient(ALPACA_API_KEY, ALPACA_SECRET_KEY, paper=True)
    return _ALPACA_TRADING_CLIENT[0]


def _alpaca_order_side(side):
    return OrderSide.BUY if str(side).lower() == 'buy' else OrderSide.SELL


def _alpaca_client_order_id(symbol, side):
    safe_symbol = ''.join(ch for ch in str(symbol).upper() if ch.isalnum() or ch in ('-', '_'))[:12] or 'UNK'
    safe_side = 'B' if str(side).lower() == 'buy' else 'S'
    return f'lpm-{safe_side}-{safe_symbol}-{int(time.time())}-{uuid.uuid4().hex[:8]}'


def _alpaca_trade_headers():
    return {
...
def _alpaca_list_orders(status='all', limit=200):
    if not _alpaca_trading_enabled():
        return []
    try:
        response = _REQUESTS_SESSION.get(
            f"{ALPACA_API_BASE_URL.rstrip('/')}/v2/orders",
            params={'status': status, 'limit': int(limit), 'direction': 'desc'},
            headers=_alpaca_trade_headers(),
            timeout=(5, 15),
        )
        ...
        print(f'  Alpaca orders poll error: {exc}')
        return []


def _alpaca_order_attr(order, name, default=None):
    if isinstance(order, dict):
        return order.get(name, default)
    return getattr(order, name, default)


def _alpaca_order_snapshot(order, source='poll', event=None):
    status = str(_alpaca_order_attr(order, 'status', '') or '').lower()
    return {
        'source': source,
        'event': str(event or status or '').lower(),
        'timestamp': _alpaca_order_attr(order, 'updated_at') or _alpaca_order_attr(order, 'submitted_at') or _alpaca_order_attr(order, 'timestamp'),
        'order_id': str(_alpaca_order_attr(order, 'id', '') or ''),
        'client_order_id': _alpaca_order_attr(order, 'client_order_id', None),
        'symbol': _alpaca_order_attr(order, 'symbol', None),
        'status': status,
        'filled_qty': str(_alpaca_order_attr(order, 'filled_qty', '') or ''),
        'filled_avg_price': str(_alpaca_order_attr(order, 'filled_avg_price', '') or ''),
        'qty': str(_alpaca_order_attr(order, 'qty', '') or ''),
        'side': str(_alpaca_order_attr(order, 'side', '') or '').lower(),
    }


def _alpaca_record_order_snapshot(pf, snapshot):
    order_map = pf.setdefault('alpaca_order_updates', {})
    order_key = snapshot['client_order_id'] or snapshot['order_id'] or snapshot['symbol'] or snapshot['event']
    existing = order_map.get(order_key)
    merged = dict(existing) if isinstance(existing, dict) else {}
    merged.update(snapshot)
    changed = merged != existing
    order_map[order_key] = merged

    matched = False
    for pending in pf.get('pending_orders', []):
        same_client = snapshot['client_order_id'] and pending.get('client_order_id') == snapshot['client_order_id']
        same_symbol = snapshot['symbol'] and pending.get('ticker') == snapshot['symbol']
        if not (same_client or same_symbol):
            continue
        before = dict(pending)
        pending['broker_event'] = snapshot['event']
        pending['broker_status'] = snapshot['status'] or snapshot['event']
        pending['broker_order_id'] = snapshot['order_id']
        pending['broker_client_order_id'] = snapshot['client_order_id']
        if snapshot['filled_qty']:
            try:
                pending['broker_filled_qty'] = float(snapshot['filled_qty'])
            except ValueError:
                pending['broker_filled_qty'] = snapshot['filled_qty']
        if snapshot['filled_avg_price']:
            try:
                pending['broker_filled_avg_price'] = float(snapshot['filled_avg_price'])
            except ValueError:
                pending['broker_filled_avg_price'] = snapshot['filled_avg_price']
        if snapshot['event'] in ('fill', 'filled', 'partial_fill', 'partially_filled'):
            pending['status'] = 'FILLED' if snapshot['event'] in ('fill', 'filled') else 'PARTIALLY_FILLED'
        elif snapshot['event'] in ('canceled', 'cancelled', 'expired', 'rejected'):
            pending['status'] = snapshot['event'].upper()
        matched = True
        changed = changed or pending != before
    return matched, merged, changed


def _alpaca_submit_market_order(symbol, qty, side, client_order_id=None):
    client = _alpaca_trading_client()
    if client is None or MarketOrderRequest is None:
        return None
    ...
    try:
        ...
        print(f'  Alpaca order error [{side} {qty} {symbol}]: {exc}')
        return None


def _alpaca_stream_enabled():
    return bool(ALPACA_STREAM_UPDATES and TradingStream is not None and _alpaca_trading_enabled())


def _alpaca_trade_update_snapshot(update):
    order = getattr(update, 'order', None)
    snapshot = _alpaca_order_snapshot(order, source='stream', event=getattr(update, 'event', None))
    snapshot['timestamp'] = getattr(update, 'timestamp', None).isoformat() if getattr(update, 'timestamp', None) else snapshot['timestamp']
    return snapshot


def _alpaca_mark_trade_update(pf, update):
    snapshot = _alpaca_trade_update_snapshot(update)
    updates = pf.setdefault('alpaca_trade_updates', [])
    updates.append(snapshot)
    if len(updates) > 50:
        del updates[:-50]
    matched, _, _ = _alpaca_record_order_snapshot(pf, snapshot)
...
def _alpaca_sync_order_statuses(pf, limit=200):
    if not _alpaca_trading_enabled():
        return pf, False
    changed = False
    for order in _alpaca_list_orders(status='all', limit=limit):
        try:
            _, _, order_changed = _alpaca_record_order_snapshot(pf, _alpaca_order_snapshot(order, source='poll'))
            changed = changed or order_changed
        except Exception as exc:
            print(f'  Alpaca order sync skip: {exc}')
    return pf, changed


async def _alpaca_trade_update_handler(update):
    try:
        pf = load_portfolio()
        matched, snapshot = _alpaca_mark_trade_update(pf, update)
        if matched:
            print(
                f"  Alpaca update: {snapshot['symbol']} {snapshot['event']} "
                f"status={snapshot['status'] or 'unknown'} qty={snapshot['filled_qty'] or snapshot['qty'] or '?'}"
            )
    except Exception as exc:
        print(f'  Alpaca stream update error: {exc}')


def _alpaca_trade_stream_runner():
    if not _alpaca_stream_enabled():
        return
    stream = None
    try:
        stream = TradingStream(api_key=ALPACA_API_KEY, secret_key=ALPACA_SECRET_KEY, paper=True)
        stream.subscribe_trade_updates(_alpaca_trade_update_handler)
        _ALPACA_TRADE_STREAM[0] = stream
        ...
        if stream is not None:
            try:
                ...
                pass
        _ALPACA_TRADE_STREAM[0] = None


def _start_alpaca_trade_stream():
    if not _alpaca_stream_enabled():
        return False
    with _ALPACA_TRADE_STREAM_LOCK:
        thread = _ALPACA_TRADE_STREAM_THREAD[0]
        if thread is not None and thread.is_alive():
            return True
        thread = threading.Thread(target=_alpaca_trade_stream_runner, name='alpaca-trade-updates', daemon=True)
        _ALPACA_TRADE_STREAM_THREAD[0] = thread
        thread.start()
    return True
...
def _alpaca_account():
    ...
    try:
        ...
        print(f'  Alpaca account error: {exc}')
        return None


def _alpaca_all_positions():
    client = _alpaca_trading_client()
    if client is None:
        return []
    try:
        ...
        print(f'  Alpaca positions error: {exc}')
        return []


def _alpaca_position_qty(symbol):
    symbol = str(symbol).strip().upper()
    for pos in _alpaca_all_positions():
        sym = str(getattr(pos, 'symbol', getattr(pos, 'asset_id', ''))).strip().upper()
        if sym == symbol:
            try:
...
def _normalize_alpaca_bars(frame):
...
def _alpaca_bars_frame(tickers, start, end):
    ...
    try:
        ...
        print(f'  Alpaca market-data error: {exc}')
        return None
...
def batch_download(tickers):
    ...
    Alpaca market data is used first when credentials are present. Yahoo Finance
    remains a fallback so the script still works when Alpaca is unavailable.
    """
    ...
    if _alpaca_data_enabled():
        try:
            start = datetime.utcnow() - timedelta(days=260)
            end   = datetime.utcnow()
            alpaca_frame = _alpaca_bars_frame(tickers, start=start, end=end)
            if isinstance(alpaca_frame, pd.DataFrame) and not alpaca_frame.empty:
                if isinstance(alpaca_frame.index, pd.MultiIndex):
                    level0 = set(alpaca_frame.index.get_level_values(0))
                    level1 = set(alpaca_frame.index.get_level_values(1)) if alpaca_frame.index.nlevels > 1 else set()
                    for t in tickers:
                        try:
                            if t in level0:
                                df = _normalize_alpaca_bars(alpaca_frame.xs(t, level=0))
                            elif t in level1:
                                df = _normalize_alpaca_bars(alpaca_frame.xs(t, level=1))
                            else:
                                continue
                            df = _clean_ohlcv(df)
                    ...
                    if len(tickers) == 1:
                        df = _clean_ohlcv(_normalize_alpaca_bars(alpaca_frame))
                        if df is not None and len(df) >= 20:
                            result[tickers[0]] = df
                if result:
                    print(f'  Alpaca: {len(result)}/{len(tickers)} tickers')
        except Exception as e:
            print(f'  Alpaca market-data fallback to Yahoo: {e}')
...
<div class="wrap">
  <h1>LLM Portfolio Manager</h1>
    <p class="sub">{datetime.now().strftime("%A %B %d %Y  %H:%M")} &nbsp;·&nbsp; {NVIDIA_MODEL} &nbsp;·&nbsp; {'Alpaca paper' if _alpaca_trading_enabled() else 'Ledger mode'}</p>
    ...
    <div style="background:#1a1a2e;color:white;border-radius:12px;padding:20px;
      ...
      <div>
        ...
        <div style="font-size:28px;font-weight:700">USD {total_val:,.0f}</div>
                <div style="color:{pf_color};font-size:15px;margin-top:2px">{total_pct:+.1f}% vs {'Alpaca paper capital' if _alpaca_trading_enabled() else 'starting capital'}</div>
      </div>
      <div>
...
def send_whatsapp(pick, ctx, ep, wl, stop_price, target_price, candidates=None, portfolio=None,
    ...
    broker_label = 'Alpaca paper' if _alpaca_trading_enabled() else 'Ledger only'
    ...
    m2 = [
        ...
        f'Broker: {broker_label} | Paper baseline ${ALPACA_PAPER_CAPITAL:,.0f}' if _alpaca_trading_enabled() else f'Broker: {broker_label}',
...
# ============================================================
# PORTFOLIO  —  paper trading engine (Alpaca paper mode uses the $100k default account)
# ============================================================

def load_portfolio():
    """Load portfolio.json from Drive, or create a fresh portfolio.

    In Alpaca paper mode, the broker-side paper account starts at $100k by
    default, so the runtime summary mirrors that balance instead of the legacy
    $10k notebook seed.
    """
    ...
    for p in paths:
        if os.path.exists(p):
            try:
                ...
                if _alpaca_trading_enabled():
                    pf['starting_capital'] = ALPACA_PAPER_CAPITAL
                    account = _alpaca_account()
                    if account is not None:
                        try:
                            pf['cash'] = round(float(getattr(account, 'cash', getattr(account, 'buying_power', ALPACA_PAPER_CAPITAL)) or ALPACA_PAPER_CAPITAL), 2)
                        except (TypeError, ValueError):
                            pf['cash'] = ALPACA_PAPER_CAPITAL
                        try:
                            pf['broker_equity'] = round(float(getattr(account, 'equity', ALPACA_PAPER_CAPITAL) or ALPACA_PAPER_CAPITAL), 2)
                        except (TypeError, ValueError):
                            pf['broker_equity'] = ALPACA_PAPER_CAPITAL
                return pf
            except Exception as e:
                print(f'  portfolio.json load error: {e}')
    # First run — create fresh portfolio
    pf = {
        'cash': ALPACA_PAPER_CAPITAL if _alpaca_trading_enabled() else STARTING_CAPITAL,
        'starting_capital': ALPACA_PAPER_CAPITAL if _alpaca_trading_enabled() else STARTING_CAPITAL,
        'positions': [],
        'closed_trades': [],
    ...
    if _alpaca_trading_enabled():
        pf['broker_equity'] = ALPACA_PAPER_CAPITAL
        print(f'  New portfolio created — starting capital: ${ALPACA_PAPER_CAPITAL:,.0f} (Alpaca paper)')
    else:
...
def open_position(pf, ticker, entry_price, amount_usd, stop, target, sector='', atr=0, nzdusd_rate=None):
    ...
    if shares < 1:
        ...
        return pf
    if _alpaca_trading_enabled():
        client_order_id = _alpaca_client_order_id(ticker, 'buy')
        broker_order = _alpaca_submit_market_order(ticker, shares, 'buy', client_order_id=client_order_id)
        if broker_order is None:
            print(f'  Alpaca BUY failed for {ticker} — ledger unchanged')
            return pf
        _alpaca_record_order_snapshot(pf, _alpaca_order_snapshot(broker_order, source='submit'))
...
def close_position(pf, ticker, exit_price, reason='hold_period', nzdusd_rate=None):
    ...
    if not pos:
        return pf
    if _alpaca_trading_enabled():
        client_order_id = _alpaca_client_order_id(ticker, 'sell')
        broker_order = _alpaca_submit_market_order(ticker, int(pos['shares']), 'sell', client_order_id=client_order_id)
        if broker_order is None:
            print(f'  Alpaca SELL failed for {ticker} — ledger unchanged')
            return pf
        _alpaca_record_order_snapshot(pf, _alpaca_order_snapshot(broker_order, source='submit'))
...
def run_screener():
    ...
    portfolio, _alpaca_sync_changed = _alpaca_sync_order_statuses(portfolio)
    if _alpaca_sync_changed:
        save_portfolio(portfolio)
    if _alpaca_stream_enabled():
        _start_alpaca_trade_stream()
    ...
    if not NVIDIA_API_KEY and not OPENROUTER_API_KEY:
        print('\n⚠️  No LLM provider credentials configured — skipping live AI screening and saving a neutral no-pick result.')
        _wa_no_pick({}, portfolio, reason='No LLM provider credentials configured')
        display_scorecard()
        return {
            'top_pick': {'ticker': 'NONE', 'confidence': 0, 'signal': 'NO PICK',
                        'reasoning': 'No LLM provider credentials configured for this run.',
                        'key_risk': 'N/A', 'sector': 'N/A', 'source': 'N/A'},
            'watch_candidates': [],
            'failure_reason': 'No LLM provider credentials configured',
        }
